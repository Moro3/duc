...
SpawImagePropDialog.init = function() 
{
  ...
  if (iProps)
  {
    ...
    if (iProps.hspace) {
      var hs = iProps.hspace;
      if (isNaN(hs) && hs.indexOf("px") != -1)
        hs = hs.substring(0, hs.length-2);
      document.getElementById('chspace').value = hs;
    }
    if (iProps.hspacer) {
      var hsr = iProps.hspacer;
      if (isNaN(hsr) && hsr.indexOf("px") != -1)
        hsr = hsr.substring(0, hsr.length-2);
      document.getElementById('chspacer').value = hsr;
    }
    if (iProps.vspace) {
      var vs = iProps.vspace;
      if (isNaN(vs) && vs.indexOf("px") != -1)
        vs = vs.substring(0, vs.length-2);
      document.getElementById('cvspace').value = vs;
    }
    ...
    if (iProps.hspace_r && iProps.hspace_r>-1) {
      document.getElementById('chspace_r').value = iProps.hspace_r;
    }
...
SpawImagePropDialog.validateParams = function()
{
  ...
  if (isNaN(parseInt(document.getElementById('chspacer').value)) && document.getElementById('chspacer').value != '')
  {
    alert(spawErrorMessages['error_hspace_nan']);
    document.getElementById('chspacer').focus();
    return false;
  }
...
SpawImagePropDialog.okClick = function() {
  ...
  if (SpawImagePropDialog.validateParams())    
  {
    ...
    if (!iProps.hspace || iProps.hspace == '')
      ...
      iProps.removeAttribute("margin-left");
    else
      iProps.style.margin-left = (document.getElementById('chspace').value)?(document.getElementById('chspace').value):'' + "px";

    iProps.hspacer = (document.getElementById('chspacer').value)?(document.getElementById('chspacer').value):'';
    //iProps.hspace_r = document.getElementById('chspace_r').value;
    if (!iProps.hspacer || iProps.hspacer == '')
      //iProps.removeAttribute("hspace");
      iProps.removeAttribute("margin-right");
    else
      iProps.style.margin-right = (document.getElementById('chspacer').value)?(document.getElementById('chspacer').value):'' + "px";
    ...
    if (!iProps.vspace || iProps.vspace == '')
      ...
      iProps.removeAttribute("margin-top");
    else
      iProps.style.margin-top = (document.getElementById('cvspace').value)?(document.getElementById('cvspace').value):'' + "px";
