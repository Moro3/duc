...
class Replaced_mod extends CI_Driver
{
    private $prefix = 'mod';          // префикс модуля
    ...
    */
    function load_data(){
    	$this->params = array('name', 'value', 'arg', 'id');
		$this->access = array(
		                 'value' => array('id', 'name', 'alias', 'mod', 'controller', 'method', 'arg'),
		);
    ...
    */
    function get_content($params){
        ...
        */
        ...
        if(is_array($data)){        	if(isset($data['mod']) && isset($data['controller']) && isset($data['method'])){        		if(!empty($data['arg'])){                    if($this->is_argument($data['arg'])){                    	$arr_args = $this->get_argument_input();
                    	if(! is_array($arr_args) || count($arr_args) == 0){
                    		$arr_args = $this->get_argument_default($data['arg']);
                    	}
                    }
                    if(!isset($arr_args)){
                    	$arr_args = $this->get_argument_default($data['arg']);
        	...
        	if(isset($arr_args)){
        		switch (count($arr_args)){
        			case  1:
        			   $content = Modules::run($data['mod']->alias.'/'.$data['controller'].'/'.$data['method'], $arr_args[0]);
        			break;
        			case  2:
        			   $content = Modules::run($data['mod']->alias.'/'.$data['controller'].'/'.$data['method'], $arr_args[0], $arr_args[1]);
        			break;
        			case  3:
        			   $content = Modules::run($data['mod']->alias.'/'.$data['controller'].'/'.$data['method'], $arr_args[0], $arr_args[1], $arr_args[2]);
        			break;
        			case  4:
        			   $content = Modules::run($data['mod']->alias.'/'.$data['controller'].'/'.$data['method'], $arr_args[0], $arr_args[1], $arr_args[2], $arr_args[3]);
        			break;
        			case  5:
        			   $content = Modules::run($data['mod']->alias.'/'.$data['controller'].'/'.$data['method'], $arr_args[0], $arr_args[1], $arr_args[2], $arr_args[3], $arr_args[4]);
        			break;
        			default:
        			   $content = Modules::run($data['mod']->alias.'/'.$data['controller'].'/'.$data['method']);
        		}
    ...
    */
    private function get_data($name){    	return Modules::run('mods/mods/get_data_template', $name);
    }
