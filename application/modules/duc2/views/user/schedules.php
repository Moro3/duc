...
if(isset($data) && is_array($data) && count($data) > 0){
	...
	foreach($data as $items){
		...
		if($numgroups_double !== $items->numgroups->name){
			//$class_group = ' border-top-color';
			$style_numgroup = 'border-width: 2px 2px 0 2px';
			$end_numgroup_duble = true;
		}
		$surname_double = $fio;
		$group_double = $items->groups__name;
		$numgroups_double = $items->numgroups->name;
        ...
        */
		...
		echo '</td>';
		echo '<td 1rowspan="2" class="numgroup '.$class_numgroup.'" style="'.$style_numgroup.'">';
			if($end_numgroup_duble === true){
	...
	echo 'Похоже расписания ещё не опубликовано.<br />Не отчаивайтесь, оно обязательно появится ;)';
	echo '</div>';
}
echo '</div>';