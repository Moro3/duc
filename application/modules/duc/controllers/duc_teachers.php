...
class Duc_teachers extends Duc {
    ...
    function listTeachers(){
        $res = Modules::run('duc/duc_teachers/MY_data_array_one',
            //key
            'main.id',
            //select
            array('main.surname', 'main.name', 'main.name2' // 'main.paid'

            ),
            //where
            false,
            //order
            array('main.surname', 'main.name', 'main.name2'),
            //separator
            ' '
        );
    ...
    //возвращает список групп
    function listTeachersEscape(){
        $res = $this->listTeachers();
    ...
    //возвращает список учителей в формате формы select
    ...
    //возвращает список учителей в формате формы select
    function selectTeachersSelect2(){
        ...
        foreach($data as $key=>$items){
            $res[$key] = '<option';
            if(!empty($items->foto)){
                $res[$key] .= ' role="option" data-img="'.$items->foto.'" value="'.$items->id.'"';
            }
            $res[$key] .= '>';
            if(!empty($items->id)){

                $res[$key] .= $items->surname.' '.$items->name.' '.$items->name2;
    ...
    */
    function tpl_teachers_list(){
    	$data['teachers'] = Modules::run(	'duc/duc_teachers/MY_data_array',
    		 					//select
    		 					array('main.id', 'main.name', 'main.surname', 'main.name', 'main.name2', 'main.experience', 'main.description', 'main.foto',// 'id_qualification', 'id_rank',
