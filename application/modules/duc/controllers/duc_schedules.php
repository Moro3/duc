...
//-------------- Загрузка таблицы jQGrid
        ...
        $this->load->view('grid/formatter/select2',
                         array('grid' => $this->table,
                                ...
                                'event' => 'jqGridAddEditAfterShowForm',
                                //'formatSelection' => '',
                                //'formatResult' => ''
                         )
        );
    ...
    */
    ...
    /**
    * Шаблон вывода расписания занятий по группе и найденных групп по названию основной группы
    *
    */
    public function tpl_schedules_groupname($id_group){
    	if(!is_numeric($id_group)) return false;
    	$group_name = Modules::run('duc/duc_groups/MY_data_row',
    	                        array('id', 'name'),
    	                        array('id' => $id_group, 'active' => 1)
    	);
    	if(!empty($group_name->name)){
	    	$data['data'] = $this->MY_data(
	    		        //select
	    		        array('main.id', 'main.id_group', 'main.active',
	    		        		 //'mon_from', 'mon_to', 'tue_from', 'tue_to', 'wed_from', 'wed_to', 'thur_from', 'thur_to', 'fri_from', 'fri_to', 'sat_from', 'sat_to', 'sun_from', 'sun_to',
