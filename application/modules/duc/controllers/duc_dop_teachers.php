...
/*
 * Класс Duc_dop_teachers
 *
 */

class Duc_dop_teachers extends Duc {
    ...
    function __construct(){
	 ...
	 /**
    * Запись дополнительных педагогов для коллектива
    * @param int - id_group
    ...
    */
    function write_teachers($id_group, $id_teachers = array()){
    	...
    	$rows = Modules::run('duc/duc_dop_teachers/MY_data_array',
    	                    //select
    	                    array('id', 'id_teacher'
    	                    ),
    	                    //where
        ...
        */
        ...
        if(is_array($cols_del) && count($cols_del) >= 1){
        	...
        	if( ! Modules::run('duc/duc_dop_teachers/MY_delete',
        	        			//where
        	        			$where
        			)
        	){
        		throw new jqGrid_Exception('Не удалось удалить педагогов у объекта "dop_teachers"');
        		return false;
        	}
        }
        //добавляем
        if(is_array($cols_add) && count($cols_add) >= 1){
        	...
        	foreach($cols_add as $item){
        		$set['id_teacher'] = $item;
        		if( ! Modules::run('duc/duc_dop_teachers/MY_insert', $set)){
         			throw new jqGrid_Exception('Не удалось добавить педагога у объекта "dop_teachers"');
         			return false;
        		}
        	}
        }
    ...
    */
    function delete_teachers($id_group){
    	...
    	// удаляем
        	...
        	if( ! Modules::run('duc/duc_dop_teachers/delete_teachers', $id_group)){
        		throw new jqGrid_Exception('Не удалось удалить педагога у объекта "dop_teachers"');
        		return false;
        	}else{
        		return true;
        	}
