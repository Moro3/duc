<?php
class grid_duc_photos extends jqGrid
{
    protected function init()
    {
        $this->CI = &get_instance();
        $this->table = 'duc_photos';
	...
	protected function renderGridUrl(){
        return '/grid/'.$this->table.'/duc/grid/';
    }
    ...
    protected function parseRow(array $r)
    {
        ...
        if(isset($r['foto_upload'])){
        	$path = $resize_config['path'].$resize_param['mini']['dir'].'/'.$r['foto_upload'];
        	$path_original = $resize_config['path'].$resize_config['dir'].'/'.$r['foto_upload'];
        	//echo $path;
    ...
    */
    protected function opDel($id) {
		...
		if(is_scalar($id))
		{
			Modules::run('duc/duc_photos/deleteObject', $id);
		...
		else
		{
			...
			Modules::run('duc/duc_photos/deleteObject', $ids);
            ...
            // проверяем был ли обновлен файл и если да,
                ...
                if(isset($_FILES[Modules::run('duc/duc_photos/formSelector', 'foto_upload')])){
                    ...
                    if(isset($data['data']['file_name']) && $row['img'] != $data['data']['file_name']){
                		Modules::run('duc/duc_photos/delete_image_object', $row['img']);
                		$upd_data['img'] = $data['data']['file_name'];
                	}
                }else{
    ...
    */
    protected function opSorter() {
        ...
        $res = Modules::run('duc/duc_photos/sorterId', $ids);
        //print_r($res);
        //exit;
