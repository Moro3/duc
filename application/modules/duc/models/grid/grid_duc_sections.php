<?php
class grid_duc_sections extends jqGrid
{
    ...
    {
        $this->CI = &get_instance();
        $this->table = 'duc_sections';

        $this->query = "
            SELECT {fields}
            FROM duc_sections AS object
            WHERE {where}
        ";
	...
	protected function renderGridUrl(){
        return '/grid/duc_sections/duc/grid/';
    }
    ...
    */
    protected function opSorter() {
        ...
        $res = Modules::run('duc/duc_sections/sorterId', $ids);
        //print_r($res);
        //exit;
